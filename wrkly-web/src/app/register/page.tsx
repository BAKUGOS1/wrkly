...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterValues) => {
    ...
    try {
      ...
      setErrorMsg(err instanceof Error ? err.message || "Failed to create account" : "Something went wrong.");
    }
  };
  ...
  const handleGoogleSuccess = async (tokenResponse: { access_token: string }) => {
    ...
    try {
      ...
      setErrorMsg(err instanceof Error ? err.message || "Google sign-up failed" : "Google sign-up failed.");
    } finally {
      ...
      <div className="w-full max-w-[420px] rounded-[16px] border border-border bg-card p-6 sm:p-10 shadow-sm">
        ...
        <div className="relative mb-[24px]">
          ...
          <div className="relative flex justify-center">
            <span className="bg-card px-[12px] text-[12px] text-muted-foreground">or sign up with email</span>
          </div>
        </div>
        ...
        <p className="text-center text-[14px] text-muted-foreground">
          ...
          <Link href="/login" className="font-medium text-primary transition-colors hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  );
